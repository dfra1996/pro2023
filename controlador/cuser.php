...
function insdatos($idusu,$pg,$arc){
	...
	if($idusu){
		$dtusu = $muser->selusu1($idusu);
	}
	$txt = '';
	$txt .= '<div class="container-fluid">';
		$txt .= vayuda("Pagina de usuarios", "Esperando mensaje...");
	...
	if ($dtusu){
			...
			foreach ($dtusu as $dt){
				$txt .= '<tr>';
					...
					$txt .= '<td>'.$dt['pfl'].'</td>';
					$txt .= '<td>'.$dt['telusu'].'</td>';
					$txt .= '<td>'.$dt['address'].'</td>';
					$txt .= '<td>'.$dt['emausu'].'</td>';
					$txt .= '<td>'.$dt['facebook'].'</td>';
					$txt .= '<td>'.$dt['instagram'].'</td>';
					$txt .= '<td>'.$dt['map'].'</td>';
					$txt .= '<td>'.$dt['about'].'</td>';
					$txt .= '<td>'.$dt['services'].'</td>';
